package hibernate.CompositePrimaryKey_NamedQueries_Mappings.NamedQueries;

@Entity
// NamedQuery gives a name for a query which we can use later.
@NamedQuery(name = "allproducts", query = "From Product")
// we can also write multiple Named Queries -
@NamedQueries({ @NamedQuery(name = "allproducts", query = "From Product"),
                @NamedQuery(name = "updateProduct", query = "update Product set name = ?1 where id = ?2")})
public class Product {

    @Id
    private int id;

    private String name;

    private String category;

    public Product() {

    }

    public Product(int id, String name, String category) {
        this.id = id;
        this.name = name;
        this.category = category;
    }

    public int getId() {
        return id;
    }

    public void setId(int id) {
        this.id = id;
    }

    public String getName() {
        return name;
    }

    public void setName(String name) {
        this.name = name;
    }

    public String getCategory() {
        return category;
    }

    public void setCategory(String category) {
        this.category = category;
    }
}
